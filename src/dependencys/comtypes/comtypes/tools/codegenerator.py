...
class Generator(object):
    ...
    def generate_code(self, items, filename):
        ...
        print(
            "from comtypes import _check_version; _check_version(%r, %s)"
            % (version, tlib_mtime),
            file=self.output,
        )
        return loops
    ...
    def type_name(self, t, generate=True):
        ...
        if isinstance(t, typedesc.PointerType):
            ...
            args = [
                self.type_name(x, generate)
                for x in [t.returns] + list(t.iterArgTypes())
            ]
    ...
    def need_OPENARRAYS(self):
        if self._OPENARRAYS_defined:
            return
        print(
            "OPENARRAY = POINTER(c_ubyte) # hack, see comtypes/tools/codegenerator.py",
            file=self.imports,
        )
    ...
    def StructureBody(self, body):
        ...
        if fields:
            ...
            for f in fields:
                ...
                if f.bits is None:
                    print(
                        "    ('%s', %s)," % (fieldname, self.type_name(f.typ)),
                        file=self.stream,
                    )
                else:
                    print(
        ...
        if methods:
            ...
            for m in methods:
                ...
                print(
                    "    COMMETHOD([], %s, '%s'," % (self.type_name(m.returns), m.name),
                    file=self.stream,
                )
                for a in m.iterArgTypes():
                    print(
    ...
    def External(self, ext):
        ...
        libdesc = str(
            ext.tlib.GetLibAttr()
        )  # str(TLIBATTR) is unique for a given typelib
    ...
    def Constant(self, tp):
        print(
            "%s = %r # Constant %s"
            % (tp.name, tp.value, self.type_name(tp.typ, False)),
            file=self.stream,
        )
        self.names.add(tp.name)
    ...
    def ComInterfaceBody(self, body):
        ...
        print(
            "## code template for %s implementation" % body.itf.name, file=self.stream
        )
        ...
        if not m.arguments:
            ...
            for typ, name, idlflags, default in m.arguments:
                ...
                if default is not None:
                    ...
                    arglist.append(
                        "( %r, %s, '%s', %r )" % (idlflags, type_name, name, default)
                    )
                else:
                    arglist.append("( %r, %s, '%s' )" % (idlflags, type_name, name))
            self.stream.write(",\n              ".join(arglist))
            print("),", file=self.stream)
